...
def main(stdscr):
    ...
    for arg in sys.argv:
        #command line arguments -u for user input -g for generator input
        if arg == '-u':
            userInput(stdscr, cells)
        
        elif arg == '-g':
            generateInput(stdscr, cells)

def generateInput(stdscr, cells):
...
def runGame(stdscr, cells):
    ...
    while(True):
        ...
        printScreen(stdscr, cellsOut)
        time.sleep(.1)

def printScreen(stdscr, cellsOut):
    #redraw window
        for i in range(0, yMax):
            for j in range(0, xMax):
